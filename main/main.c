...
static void show_words(const char *mnemonic, const char *title, int total)
{
    ...
    for (int page = 0; page * 4 < n; ) {
        char hdr[22], l[4][22];
        snprintf(hdr, sizeof(hdr), "%s %d/%d", title, page + 1, (n + 3) / 4);
        for (int i = 0; i < 4; i++) {
            int w = page * 4 + i;
            if (w < n) snprintf(l[i], sizeof(l[i]), "%2d. %s", w + 1, words[w]);
            else       l[i][0] = '\0';
        }
        screen(hdr, l[0], l[1], l[2], l[3],
               page > 0 ? "toque=atras manter=ok" : "manter = continuar");
...
void app_main(void)
{
    ...
    /* 4. aquecimento: toques suficientes para fechar a fonte de timing, que tem
